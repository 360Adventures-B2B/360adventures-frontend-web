...
const BookingSummary: React.FC<BookingSummaryProps> = ({
  ...
  const grandTotal = bookingData.reduce(
      ...
      {bookingData.map((item, index) => {
        ...
        return (
          <div key={item.ulid} className="mb-4 border-b pb-4">
            <BookingItemCard item={item} /> {/* tombol ada di bawah pickup */}
            {item.package?.pickup_included === true &&
              (form ? (
                ...
                <div className="flex items-center gap-2 mt-3">
                  ...
                  <span className="font-medium text-neutral-900 dark:text-neutral-100">
                    {item.pickup_location || (
                      <span className="italic text-neutral-500">Not set</span>
                    )}
                  </span>
                </div>
              ))}
            {/* Download Button di bawah Pickup Location */}
            {enableDownload && (
              <div className="mt-4">
                <ButtonPrimary
                  loading={isLoadingDownload}
                  onClick={() => {
                    handleDownload(item.ulid);
                  }}
                  fontSize="text-sm"
                  sizeClass="px-4 py-2"
                  className="w-full"
